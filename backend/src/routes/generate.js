...
async function buildCandidates(listId, selectedColors) {
  ...
  const validPairs = selectedColors === null
    ? allPairs
    : allPairs.filter(([a, b]) => {
        const colorless = a.color_identity.length === 0 ? a : b.color_identity.length === 0 ? b : null;
        if (colorless) {
          // One card can take any single colour (e.g. Clara, some Baldur's Gate cards);
          // valid when the other card leaves at most one gap in the selected filter
          const fixed = colorless === a ? b : a;
          return selectedColors.filter(c => !fixed.color_identity.includes(c)).length <= 1;
        }
